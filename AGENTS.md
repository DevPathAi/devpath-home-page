...
- 공개 4단계는 `진단 → 로드맵 → AI 멘토 → 경로 보정`이다. 경로 보정은 `막힌 순간을 기억하고 다음 미션을 바꿉니다.`보다 넓게 주장하지 않는다.
...
- 게스트 진단 CTA는 앱 `/diagnostic`, AI 멘토 초대 CTA는 앱 `/login` → OAuth → 기존 `BETA_PENDING` 흐름을 사용한다.
...
- Footer는 제품/회사/법적 3열과 상호·대표·사업자등록번호·소재지·이메일 회사 정보 블록을 유지한다.
