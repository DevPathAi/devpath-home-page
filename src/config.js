// 런타임 설정 (T3/T4). index.html 또는 배포 환경에서 window.LEVA_CONFIG 로 덮어쓴다.
...
const overrides = (typeof window !== 'undefined' && window.LEVA_CONFIG) || {};

export const config = {
  // 동일 출처 CF Pages Function 프록시(권장) — 함수가 서버측에서 Apps Script로 포워딩.
  // window.LEVA_CONFIG.formEndpoint 로 Apps Script /exec URL 직접 지정도 가능.
